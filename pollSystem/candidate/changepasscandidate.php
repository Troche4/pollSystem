...
//retrive candidate details from the tbmembers table
...
if($row)
 ...
 $stdId = $row['candidate_id']; 
  $encpass= $row['candidate_password'];
}
?>
<?php
// updating sql query
...
if($encpass == $oldpass)
{
  ...
  {
    $newpassword = $newpass; 
    $sql = mysqli_query($con,"UPDATE tbcandidates SET candidate_password='$newpassword' WHERE candidate_id = '$myId'" );
...
<form action="changepasscandidate.php?id=<?php echo $_SESSION['candidate_id']; ?>" method="post">
<table align="center">
...
<tr><td>&nbsp;</td></td><td><input type="submit" name="changepasscandidate" value="Update Profile"></td></tr>
</table>
</form>
<hr>
</div>
